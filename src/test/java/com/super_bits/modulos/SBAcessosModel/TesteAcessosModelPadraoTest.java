...
public class TesteAcessosModelPadraoTest extends TesteJunitSBPersistencia {
    ...
    @Test
    public void teste() {
        List<ComoAcaoDoSistema> acoesDoSistema = MapaAcoesSistema.getListaTodasAcoes();

        for (FabTipoAcaoSistemaGenerica tipoAcao : FabTipoAcaoSistemaGenerica.values()) {

            Optional<ComoAcaoDoSistema> accaoDoTipo = acoesDoSistema.stream().filter(acao -> acao.getTipoAcaoGenerica().equals(tipoAcao)).findFirst();
